...
void VHTausAnalysis::ExecuteEvent( const SInputData&, Double_t ) throw( SError ) 
{
    ...
    for (int iCat = 0; iCat < 4; ++iCat)
    {
        while (true)
        {
            ...
            if (event_HT<700)
            {
                for (unsigned int i = 0; i<m_catNames.size(); ++ i) 
                    fillEff(m_catNames[i], false);
                return;
            }
...
////////////////////////////////////////////////////////////////////////////////////////////
            ...
            for ( int i = 0; i < (m_jetAK8.N); ++i ) 
            {
                ...
                if (passCuts && isJetID && massWindow && isMetAngle)
                {
                    if (is_btag_SR){
                        if (isHbbtag) 
...
void VHTausAnalysis::bookHistograms( const TString& directory ) {
    ...
    if (m_isTTbar){
        ...
        Book( TEfficiency("eff_600_800","",1,600,800), directory );
        Book( TEfficiency("eff_800_1200","",1,800,1200), directory );
        Book( TEfficiency("eff_1200_2500","",1,1200,2500), directory );
        Book( TEfficiency("eff_2500_Inf","",1,2500,1000000), directory );
    }
}

void VHTausAnalysis::fillEff (const TString& directory, const bool pass )
{
    //Retrieve<TH2F>("tauIso_vs_Hbbtag_2_pt500_650", directory )->Fill
    Retrieve<TEfficiency>("eff_600_800",directory)->Fill(pass, event_HT);
    Retrieve<TEfficiency>("eff_800_1200",directory)->Fill(pass, event_HT);
    Retrieve<TEfficiency>("eff_1200_2500",directory)->Fill(pass, event_HT);
    Retrieve<TEfficiency>("eff_2500_Inf",directory)->Fill(pass, event_HT);
}
                                    ...
                                    TLorentzVector sumTau, TLorentzVector sumAll)
{
    ...
    if (m_isTTbar)
    {
        Hist( "top_mass", directory )->Fill(Mtt,b_weight);
