...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (unsigned int i = 0; i < jetAK8_TLV.size(); ++i ) 
    {
        ...
        if (passCuts && isJetID)
        {
            ...
            //     if ( (myjetPuppi.jetAK8_puppi_pt() > m_jetPtCut) && (fabs(myjetPuppi.jetAK8_puppi_eta()) < m_jetEtaCut) ) { // some margin...
            ...
            //             if (puppijet.DeltaR(jetAK8_TLV[i]) <  DR_puppijet_ak8){

            //                 DR_puppijet_ak8=puppijet.DeltaR(jetAK8_TLV[i]);
            //                 goodPuppiJet=myjetPuppi;
            //                 FoundPuppiJet=true;
            //                 // std:: cout << "myjetPuppi.jetAK8_puppi_pt() " << myjetPuppi.jetAK8_puppi_pt() << "ak8 Jet.pt() " <<  Jet.pt() << std::endl;
            //             } // if match

            //     }// if puppi jet passes (looser) cuts

            // } // loop on puppi AK08

            // // if (FoundPuppiJet) cout<<"yes"<<endl;
            // // else cout<<"no"<<endl;

            // if ( ! FoundPuppiJet ) continue;
            // // {
            // //     goodAK8_softdrop_mass.push_back(jetAK8_softdrop_mass[i]);
            // //     continue;
            // // }
            ...
            // // fromhere
            // // cout<<puppiCorr<<endl; return;

            // double softdrop_mass_smeared = getPuppi_sd_mass_ScaleAndResolution(true, goodPuppiJet.jetAK8_puppi_softdrop_mass() *puppiCorr, 
            //                                                                    puppijet_tlv,jetAK8_TLV[i],jetAK8_jer_sigma_pt[i], 0, 0);


            // // cout<<goodPuppiJet.jetAK8_puppi_softdrop_mass()<<"\t"<<puppijet_tlv.Pt()<<"\t"<<jetAK8_TLV[i].Pt()<<
            // //     "\t"<<jetAK8_jer_sigma_pt[i]<<endl;
            // // return;

            // // cout<<softdrop_mass_smeared<<endl; return;

            // goodAK8_TLV.push_back(jetAK8_TLV[i]);
            // goodAK8_Hbbtag.push_back(jetAK8_Hbbtag[i]);
            // goodAK8_softdrop_mass.push_back(jetAK8_softdrop_mass[i]);
            // goodAK8_pruned_mass.push_back(jetAK8_pruned_mass[i]);
            // goodAK8_softdrop_mass_corr.push_back( goodPuppiJet.jetAK8_puppi_softdrop_mass() *puppiCorr );
            // goodAK8_softdrop_mass_PUPPI.push_back( goodPuppiJet.jetAK8_puppi_softdrop_mass() );
            // goodAK8_softdrop_mass_smeared.push_back(softdrop_mass_smeared);

            // goodAK8_DR_puppi.push_back(DR_puppijet_ak8);
            // // goodAK8_softdrop_mass.push_back(softdrop_mass_smeared);


            // // cout<<"old mass = "<<jetAK8_softdrop_mass[i]<<"\t new mass = "<<softdrop_mass_smeared<<endl;
    ...
    for (unsigned int i = 0; i < goodAK8_TLV.size(); ++i) 
    {
        ...
        if ( (goodAK8_TLV[i].Pt() > goodAK08TLV_sel.Pt()) && isMetAngle)
        {
            ...
            // jet_DR_puppi = goodAK8_DR_puppi[i] ;
        }
    }
    ...
    // if (FoundPuppiJet) cout<<"found_puppi: yes"<<endl;
    // else cout<<"found_puppi: no"<<endl;
    ...
    double softdrop_mass_smeared = -1.;

    if (!m_isData) softdrop_mass_smeared = getPuppi_sd_mass_ScaleAndResolution(true, goodPuppiJet.jetAK8_puppi_softdrop_mass() *puppiCorr, 
                                                                               puppijet_tlv,goodAK08TLV_sel,jetJerSigmaPt, 0, 0);
    



    jetSoftdropMass_PUPPI = goodPuppiJet.jetAK8_puppi_softdrop_mass();
    jetSoftdropMass_corr = goodPuppiJet.jetAK8_puppi_softdrop_mass() *puppiCorr ; 
    jetSoftdropMass_smeared = softdrop_mass_smeared; 
    ...
    if (!m_isData) jetMassWindow = (jetSoftdropMass_smeared > m_jetMassMin) && (jetSoftdropMass_smeared < m_jetMassMax);
    else jetMassWindow = (jetSoftdropMass_corr > m_jetMassMin) && (jetSoftdropMass_corr < m_jetMassMax);
...
void VHTausAnalysis::clearBranches() {
    ...
    goodAK8_softdrop_mass_corr.clear() ;
    goodAK8_softdrop_mass_PUPPI.clear() ;
    goodAK8_softdrop_mass_smeared.clear() ;
    goodAK8_DR_puppi.clear() ;
    ...
    jetPrunedMass = 0. ;
    jetSoftdropMass = 0. ;
    jetSoftdropMass_corr = 0. ;
    jetSoftdropMass_PUPPI = 0. ;
    jetSoftdropMass_smeared = 0. ;
    jetJerSigmaPt = 0. ;
    jet_DR_puppi = -1. ;
    jetHbbtag = -2.;
    tau1_rawIso = -2;
...
void VHTausAnalysis::bookHistograms( const TString& directory ) {
    ...
    Book ( TH1F("jet_mass","jet softdrop mass with correction and smearing;jet softdrop mass [GeV]",50,0,250), directory);

    Book ( TH1F("jet_softdrop_mass","jet softdrop mass;jet softdrop mass [GeV]",50,0,250), directory);
    Book ( TH1F("jet_pruned_mass","jet pruned mass;jet pruned mass [GeV]",50,0,250), directory);
    Book ( TH1F("jet_softdrop_mass_PUPPI","jet softdrop mass corr;jet softdrop mass [GeV]",50,0,250), directory);
    Book ( TH1F("jet_softdrop_mass_corr","jet softdrop mass corr;jet softdrop mass [GeV]",50,0,250), directory);
    Book ( TH1F("jet_softdrop_mass_smeared","jet softdrop mass smeared;jet softdrop mass [GeV]",50,0,250), directory);
    Book ( TH1F("jet_DR_puppi_hist","CHS - PUPPI delta R;DR",100,0,1), directory);
...

{
    ...
    if (!m_isData) Hist( "jet_mass", directory )->Fill( jetSoftdropMass_smeared , b_weight );
    else Hist( "jet_mass", directory )->Fill( jetSoftdropMass_corr , b_weight );

    Hist( "jet_softdrop_mass", directory )->Fill( jetSoftdropMass , b_weight );
    Hist( "jet_pruned_mass", directory )->Fill( jetPrunedMass , b_weight );
    Hist( "jet_softdrop_mass_PUPPI", directory )->Fill( jetSoftdropMass_PUPPI , b_weight );
    Hist( "jet_softdrop_mass_corr", directory )->Fill( jetSoftdropMass_corr , b_weight );
    Hist( "jet_softdrop_mass_smeared", directory )->Fill( jetSoftdropMass_smeared , b_weight );
    Hist( "jet_DR_puppi_hist", directory )->Fill( jet_DR_puppi , b_weight );
