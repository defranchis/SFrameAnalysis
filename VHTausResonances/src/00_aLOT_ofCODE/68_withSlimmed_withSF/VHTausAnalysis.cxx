...
////////////////////////////////////////////////////////////////////////////////////////////
            ...
            std::vector<UZH::Tau> goodSlimmed;
            ...
            for ( int i = 0; i<(m_tau.N); ++i )
            {
                ...
                if (passCuts && isTauDM && againstElectron && againstMuon)
                {
                    if (is_tauiso_SR)
                    {
                        if (isTauIso)
                        {
                            if (isTauBoosted) goodTaus.push_back(mytau);
                            else goodSlimmed.push_back(mytau);
                        }
                    }
                    ...
                    if ( deltaPhi < m_tautauMetDeltaPhiMax )
                    {
                        ...
                        if ( (py2*px1 - px2*py1) != 0 && (px1 != 0) )
                        {
                            ...
                            if ((sumPT >= sumPT_leading) && (sumPT > m_tautauPtCut) && (deltaR_tautau < m_tautauDRMax)
                                ...
                                && (mass > m_tautauMassMin) && (mass < m_tautauMassMax) )
            ...
            if (!foundTauPair)
            {
                ...
                if (goodSlimmed.size() < 2) break;
                // std::cout<<"i'm in slimmed taus loop"<<std::endl;
        
                sumPT_leading = 0;
                iTau_sel = -1;
                jTau_sel = -1;

                for (unsigned int iTau=0; iTau<goodSlimmed.size(); ++iTau)

                    for (unsigned int jTau=iTau+1; jTau<goodSlimmed.size(); ++jTau)
                    {
                        TLorentzVector tau1_TLV, tau2_TLV, sumTLV;
            
                        tau1_TLV.SetPtEtaPhiE( goodSlimmed.at(iTau).pt(), goodSlimmed.at(iTau).eta(), 
                                               goodSlimmed.at(iTau).phi() ,goodSlimmed.at(iTau).e() );
                        tau2_TLV.SetPtEtaPhiE( goodSlimmed.at(jTau).pt(), goodSlimmed.at(jTau).eta(), 
                                               goodSlimmed.at(jTau).phi() ,goodSlimmed.at(jTau).e() );
                        ...
                        if ( deltaPhi < m_tautauMetDeltaPhiMax )
                        {
                            ...
                            if ( (py2*px1 - px2*py1) != 0 && (px1 != 0) )
                            {
...

                                {
                                    ...
                                    if (goodSlimmed.at(iTau_sel).pt()>goodSlimmed.at(jTau_sel).pt())
                                    {
                                        selectedTau_1 = goodSlimmed.at(iTau_sel);
                                        selectedTau_2 = goodSlimmed.at(jTau_sel);
                                    }

                                    else
                                    {
                                        selectedTau_1 = goodSlimmed.at(jTau_sel);
                                        selectedTau_2 = goodSlimmed.at(iTau_sel);
                                    }
            ...
            }// if not found boosted taus


            if (iTau_sel>=0 && jTau_sel>=0) foundTauPair = true;
            if (!foundTauPair) break;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if (!m_isData)
            {
                ...
                if (m_isSignal)
                {
                    if (is_btag_SR)
                    {
                        if      ( pt>=300. && pt<400. ) b_weight *= SF_btag_loose_300_400;
                        else if ( pt>=400. && pt<500. ) b_weight *= SF_btag_loose_400_500; 
                        else if ( pt>=500. && pt<600. ) b_weight *= SF_btag_loose_500_600; 
                        else if ( pt>=600.            ) b_weight *= SF_btag_loose_600_700; 
                        else                            b_weight *= 1.;
                    }

                    else 
                    {
                        if      ( pt>=300. && pt<400. ) b_weight *= 2.- SF_btag_loose_300_400;
                        else if ( pt>=400. && pt<500. ) b_weight *= 2.- SF_btag_loose_400_500; 
                        else if ( pt>=500. && pt<600. ) b_weight *= 2.- SF_btag_loose_500_600; 
                        else if ( pt>=600.            ) b_weight *= 2.- SF_btag_loose_600_700; 
                        else                            b_weight *= 1.;
                    }
                ...
                else if (m_isTTbar)
                {
                    if (is_btag_SR)
                    {
                        if      ( pt>=300. && pt<400. ) b_weight *= mistag_SF_btag_loose_300_400;
                        else if ( pt>=400. && pt<550. ) b_weight *= mistag_SF_btag_loose_400_550; 
                        else if ( pt>=550.            ) b_weight *= mistag_SF_btag_loose_550_Inf; 
                        else                            b_weight *= 1.;
                    }

                    else
                    {
                        if      ( pt>=300. && pt<400. ) b_weight *= 2.- mistag_SF_btag_loose_300_400;
                        else if ( pt>=400. && pt<550. ) b_weight *= 2.- mistag_SF_btag_loose_400_550; 
                        else if ( pt>=550.            ) b_weight *= 2.- mistag_SF_btag_loose_550_Inf; 
                        else                            b_weight *= 1.;
                    }
