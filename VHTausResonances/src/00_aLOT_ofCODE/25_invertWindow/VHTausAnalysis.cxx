...
/*
  ...
  Tighter selections - no tau IDs
  INVERTED MASS WINDOW
...

{
    ...
    DeclareProperty( "jetMetDeltaPhiMin",        m_jetMetDeltaPhiMin  = TMath::Pi()*3./4.);
    ...
    DeclareProperty( "jetMassMax",               m_jetMassMax         = 70. );


    DeclareProperty( "tauPtCut",                 m_tauPtCut              = 50. );
    DeclareProperty( "tauEtaCut",                m_tauEtaCut             = 2.4 );
    DeclareProperty( "tautauPtCut",              m_tautauPtCut           = 400.);
    DeclareProperty( "tautauDRMax",              m_tautauDRMax           = 1.  );
    DeclareProperty( "tautauDRMin",              m_tautauDRMin           = .1  );
    DeclareProperty( "tautauMetDeltaPhiMax",     m_tautauMetDeltaPhiMax  = TMath::Pi()/4.);
            ...
            if ( deltaPhi < m_tautauMetDeltaPhiMax )
            {
                ...
                if ( (py2*px1 - px2*py1) != 0 && (px1 != 0) )
                {
                    ...
                    if ( (sumPT >= sumPT_leading) && (sumPT > m_tautauPtCut) && (deltaR_tautau < m_tautauDRMax) &&
                         ...
                         (resonanceMass > m_tautauJetMassMin ) && (deltaR_tautau > m_tautauDRMin) ) 
...
void VHTausAnalysis::bookHistograms( const TString& directory ) {
    ...
    Book ( TH1F("jet_mass","jet pruned mass (with JEC);jet mass (GeV)",40,0,200), directory);
    ...
    //Book ( TH1F("jet_phi","jet angle;jet #phi",20,-TMath::Pi(),TMath::Pi()), directory);
    ...
    //Book ( TH1F("tau1_phi","leading tau azimuthal angle;leading tau #phi",20,-TMath::Pi(),TMath::Pi()), directory);
    ...
    Book ( TH1F("MET_ET","missing energy ET;missing energy E_{T} (GeV)",30,0,2000), directory);
    ...
    Book ( TH1F("resonance_mass","tautau-jet invariant mass (coll. nu);m(#tau#tau,jet) (GeV)",25,0,5000), directory);
