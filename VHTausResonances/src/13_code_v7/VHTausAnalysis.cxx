...
void VHTausAnalysis::BeginCycle() throw( SError ) {
    ...
    if (!m_isRunH) m_triggerNames.push_back("PFHT800_v") ;
    else m_triggerNames.push_back("PFHT900_v") ;
    ...
    if ( (m_isData && (!m_isRunH)) || (!m_isData) )
    {
        m_catNames.push_back("HT800_tautau_NoWindow");
        m_catNames.push_back("HT800_0_SR");
        m_catNames.push_back("HT800_1_bveto");
        m_catNames.push_back("HT800_2_tauveto");
        m_catNames.push_back("HT800_3_allveto");
        m_catNames.push_back("HT800_4_checkBtag");
    }

    if ( (m_isData && m_isRunH) || (!m_isData) )
    {
        m_catNames.push_back("HT900_tautau_NoWindow");
        m_catNames.push_back("HT900_0_SR");
        m_catNames.push_back("HT900_1_bveto");
        m_catNames.push_back("HT900_2_tauveto");
        m_catNames.push_back("HT900_3_allveto");
        m_catNames.push_back("HT900_4_checkBtag");
    }
...

{
    ...
    double a_factor = 1.;
    if ( directory.Contains("HLT900") && !m_isData ) a_factor = HT900_factor;

    if (m_isTTbar) Hist( "lheHT", directory )->Fill( event_lheHT );

    Hist( "HT", directory )->Fill( event_HT , b_weight * a_factor);
    Hist( "n_PV", directory )->Fill( m_eventInfo.PV_N , b_weight * a_factor);
    
    Hist( "jet_mass", directory )->Fill( jetSoftDropMass , b_weight * a_factor);
    Hist( "jet_pT", directory )->Fill( goodAK08TLV_sel.Pt() , b_weight * a_factor);
    Hist( "jet_eta", directory )->Fill( goodAK08TLV_sel.Eta() , b_weight * a_factor);
    Hist( "jet_Hbbtag", directory )->Fill( jetHbbtag , b_weight * a_factor);

    Hist( "tau1_pT", directory )->Fill( tau1_TLV_sel.Pt() , b_weight * a_factor);
    Hist( "tau1_eta", directory )->Fill( tau1_TLV_sel.Eta() , b_weight * a_factor);
    Hist( "tau1_iso", directory )->Fill( tau1_rawIso , b_weight * a_factor);
    Hist( "tau2_pT", directory )->Fill( tau2_TLV_sel.Pt() , b_weight * a_factor);
    Hist( "tau2_eta", directory )->Fill( tau2_TLV_sel.Eta() , b_weight * a_factor);
    Hist( "tau2_iso", directory )->Fill( tau2_rawIso , b_weight * a_factor);
    Hist( "sum_iso", directory )->Fill( tau1_rawIso + tau2_rawIso , b_weight * a_factor);

    Hist( "MET_ET", directory )->Fill( met_et , b_weight * a_factor);
    Hist( "MET_px", directory )->Fill( met_px , b_weight * a_factor);
    Hist( "MET_py", directory )->Fill( met_py , b_weight * a_factor);
    ...
    Hist( "tautau_mass", directory )->Fill(tautauMass, b_weight * a_factor);
    Hist( "tautau_pT", directory )->Fill(tautauPT, b_weight * a_factor);
    Hist( "tautau_eta", directory )->Fill(tautauEta, b_weight * a_factor);
    Hist( "tautau_DR", directory )->Fill(tautauDR, b_weight * a_factor);
    Hist( "taujet_DR", directory )->Fill(tauJetDR, b_weight * a_factor);
    Hist( "resonance_mass", directory )->Fill(tauJetMass, b_weight * a_factor);
    Hist( "resonance_mass_bin", directory )->Fill(tauJetMass, b_weight * a_factor);

    TVector2 v2_met (met_px, met_py);
    TVector2 v2_taus (sumTau_TLV_sel.Px(),sumTau_TLV_sel.Py());
    TVector2 v2_jet (goodAK08TLV_sel.Px(),goodAK08TLV_sel.Py());

    Hist( "tautaujet_DP", directory )->Fill(fabs(v2_jet.DeltaPhi(v2_taus)),b_weight * a_factor);    
    Hist( "jetmet_DP", directory )->Fill(fabs(v2_met.DeltaPhi(v2_jet)),b_weight * a_factor);
    Hist( "tautaumet_DP", directory )->Fill(fabs(v2_met.DeltaPhi(v2_taus)),b_weight * a_factor);
...
void VHTausAnalysis::applyTurnOn (){
    ...
    if (event_HT<1500.){
        ...
        if ( do_turnon_shift == 0 ) factor_800 = trig_eff_HT800->GetEfficiency(trig_eff_HT800->FindFixBin(event_HT));
        else if ( do_turnon_shift == 1 ) factor_800 = trig_eff_HT800->GetEfficiency(trig_eff_HT800->FindFixBin(event_HT)) + 
                                            trig_eff_HT800->GetEfficiencyErrorUp(trig_eff_HT800->FindFixBin(event_HT));
        else if ( do_turnon_shift == -1 ) factor_800 = trig_eff_HT800->GetEfficiency(trig_eff_HT800->FindFixBin(event_HT)) - 
                                            trig_eff_HT800->GetEfficiencyErrorLow(trig_eff_HT800->FindFixBin(event_HT));

        b_weight *= factor_800;

        double factor_900 = 1.;
        
        if ( do_turnon_shift == 0 ) factor_900 = trig_eff_HT900->GetEfficiency(trig_eff_HT900->FindFixBin(event_HT));
        else if ( do_turnon_shift == 1 ) factor_900 = trig_eff_HT900->GetEfficiency(trig_eff_HT900->FindFixBin(event_HT)) + 
                                            trig_eff_HT900->GetEfficiencyErrorUp(trig_eff_HT900->FindFixBin(event_HT));
        else if ( do_turnon_shift == -1 ) factor_900 = trig_eff_HT900->GetEfficiency(trig_eff_HT900->FindFixBin(event_HT)) - 
                                            trig_eff_HT900->GetEfficiencyErrorLow(trig_eff_HT900->FindFixBin(event_HT));

        HT900_factor = factor_900 / factor_800 ;
