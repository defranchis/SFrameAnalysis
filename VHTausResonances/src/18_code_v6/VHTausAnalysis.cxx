...
/*
  ...
  - MEDIUM DOUBLE-B TAGGER
  - ONE TAU MEDIUM ISOLATED
  - THE OTHER VLOOSE ISOLATED
...
void VHTausAnalysis::ExecuteEvent( const SInputData&, Double_t ) throw( SError ) 
{
    ...
    for ( int i = 0; i < (m_jetAK4.N); ++i ) 
    {
        ...
        {
        case (1):
            pt *= myjet.jecUp()/myjet.jec();
            break;

        case (-1):
            pt *= myjet.jecDown()/myjet.jec();
            break;
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for ( int i = 0; i < (m_jetAK8.N); ++i ) 
    {
        ...
        bool passCuts =  (pt > m_jetPtCut) && (fabs(myjet.eta()) < m_jetEtaCut);
    ...
    for (unsigned int i = 0; i < goodFatJets.size(); ++i) 
    {
        ...
        {
        case (1):
            jetTLV *= goodFatJets.at(i).jecUp()/goodFatJets.at(i).jec();
            break;

        case (-1):
            jetTLV *= goodFatJets.at(i).jecDown()/goodFatJets.at(i).jec();
            break;
        ...
        if ( (jetTLV.Pt() > leadingPT) && isMetAngle)
        {
    ...
    // cout<<endl;
    // cout<<"jecUp = "<<selectedJet.jecUp()<<endl;
    // cout<<endl;
    ...
    case (1):
        jetAK08TLV_sel *= selectedJet.jecUp()/selectedJet.jec();
        break;

    case (-1):
        jetAK08TLV_sel *= selectedJet.jecDown()/selectedJet.jec();
        break;
    ...
    double jetPrunedMass = selectedJet.pruned_massCorr();
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (!m_isData)
    {
        ...
        case (1):
            pt *= selectedJet.jecUp()/selectedJet.jec();
            break;

        case (-1):
            pt *= selectedJet.jecDown()/selectedJet.jec();
            break;
        ...
        fillHistograms(m_catNames[0], selectedJet, selectedTau_1, selectedTau_2, goodMet, jetAK08TLV_sel, sumTau_TLV_sel, sumAll_TLV_sel);
    ...
    fillHistograms(m_catNames[iCat], selectedJet, selectedTau_1, selectedTau_2, goodMet, jetAK08TLV_sel, sumTau_TLV_sel, sumAll_TLV_sel);
...
void VHTausAnalysis::fillHistograms(const TString& directory, const UZH::Jet& Jet, const UZH::Tau Tau1, const UZH::Tau Tau2, const UZH::MissingEt MET,
                                    TLorentzVector jet_jec, TLorentzVector sumTau, TLorentzVector sumAll)
{
    ...
    Hist( "jet_pT", directory )->Fill( jet_jec.Pt() , b_weight);
    ...
    float tauJetDR = sumTau.DeltaR(jet_jec);
    float tauJetDEta = fabs(sumTau.Eta()-jet_jec.Eta());
    ...
    TVector2 v2_jet (jet_jec.Px(),jet_jec.Py());
