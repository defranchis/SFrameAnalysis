...
int main ( int argc, char * argv[] ){
    ...
    while (!inSig.eof()){
        ...
        for (unsigned int iBin = 0; iBin < massMin.size() -1; ++iBin)
        {
            ...
            outDatacard<<"kmax "<<4+syst.size()<<"  number of nuisance parameters (sources of systematical uncertainties)"<<endl;
            outDatacard<<"------------"<<endl;
            ...
            outDatacard<<"process\tsignal\tTT\tDYJets\tWJets"<<endl;
            ...
            outDatacard<<"rate\t"<<sig_integr.at(iBin)*lumi_factor/100.<<"\t"<<TT_bkg.at(iBin)*lumi_factor<<"\t"
                       <<DY_bkg.at(iBin)*lumi_factor<<"\t"<<WJets_bkg.at(iBin)*lumi_factor<<endl;
