...

{
    ...
    DeclareProperty( "METCut",                   m_metCut             = 10. );
    ...
    // DeclareProperty( "tautauMassMin",            m_tautauMassMin         = 50. );
   ...
   // DeclareProperty( "JSONName",                 m_jsonName             = std::string (std::getenv("SFRAME_DIR")) + "/../GoodRunsLists/JSON/Cert_271036-277148_13TeV_PromptReco_Collisions16_JSON_NoL1T.txt" );
    ...
    // trig_turnon_eff = new TFile("~/HH/8_X_Framework/Analysis/SFrameAnalysis/VHTausResonances/trigger_turnon/HLT_HT800or900_wMitigation_new.root");
    // trig_eff = (TEfficiency*) trig_turnon_eff->GetObjectChecked("trigger_eff_vs_HT/eff_HT_thick","TEfficiency");

    // file_HT_corr = new TFile("~/HH/8_X_Framework/Analysis/SFrameAnalysis/VHTausResonances/trigger_turnon/HT_correction.root");
    // hist_HT_corr = (TH1D*) file_HT_corr->GetObjectChecked("HT_correction","TH1D");

    // file_HbbTag_corr = new TFile("~/HH/8_X_Framework/Analysis/SFrameAnalysis/VHTausResonances/corrections/HbbTag_correction.root");
    // hist_HbbTag_corr = (TH1D*) file_HbbTag_corr->GetObjectChecked("HbbTag_correction","TH1D");
...
void VHTausAnalysis::BeginCycle() throw( SError ) {
    ...
    m_catNames.push_back("tautau_bveto");
...
void VHTausAnalysis::ExecuteEvent( const SInputData&, Double_t ) throw( SError ) 
{
    ...
    //         double deltaR = ele_TLV.DeltaR(tau_TLV.at(iTau));
    //         if ( deltaR > 0.05 && deltaR < 0.3 ) {

    //             foundTau = true;
    //             eleIso = ( (myelectron.SemileptonicPFIso()-tau_TLV.at(iTau).Pt()) / myelectron.pt() );
    //             break;
    //             } 

    //     } //loop on boosted taus

    //     if (!foundTau) {

    //         for (size_t iTau = 0; iTau < slimmed_TLV.size(); ++iTau){

    //             double deltaR = ele_TLV.DeltaR(slimmed_TLV.at(iTau));
    //             if ( deltaR > 0.05 && deltaR < 0.3 ) {

    //                 foundTau = true;
    //                 eleIso = ( (myelectron.SemileptonicPFIso()-slimmed_TLV.at(iTau).Pt()) / myelectron.pt() );
    //                 break;
    //             } 
    ...
    //         double deltaR = mu_TLV.DeltaR(tau_TLV.at(iTau));
    //         if ( deltaR > 0.05 && deltaR < 0.4 ) {

    //             foundTau = true;
    //             muIso = ( (mymuon.SemileptonicPFIso()-tau_TLV.at(iTau).Pt()) / mymuon.pt() );
    //             break;
    //             } 
    ...
    //             double deltaR = mu_TLV.DeltaR(slimmed_TLV.at(iTau));
    //             if ( deltaR > 0.05 && deltaR < 0.4 ) {

    //                 foundTau = true;
    //                 muIso = ( (mymuon.SemileptonicPFIso()-slimmed_TLV.at(iTau).Pt()) / mymuon.pt() );
    //                 break;
    //             } 
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (unsigned int i = 0; i < jetAK8_TLV.size(); ++i ) 
    {
        ...
        if (passCuts && isJetID)
        {
            ...
            goodAK8_softdrop_mass.push_back(jetAK8_softdrop_mass[i]);
        }
    ...
    for (unsigned int i = 0; i < goodAK8_TLV.size(); ++i) 
    {
        ...
        if ( (goodAK8_TLV[i].Pt() > goodAK08TLV_sel.Pt()) && isMetAngle)
        {
            ...
            jetSoftdropMass = goodAK8_softdrop_mass[i]; 
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // if (jetSidebandMassWindow) shiftCat = 8;
    ...
    // if (!m_isData){
    //     fillHistograms(m_catNames[5]);
    //     fillHistograms(m_catNames[iCat+5]);
    // }
...
void VHTausAnalysis::bookHistograms( const TString& directory ) {
    ...
    Book ( TH1F("resMass_n10","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_n09","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_n08","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_n07","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_n06","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_n05","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_n04","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_n03","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_n02","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_n01","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_00","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_01","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_02","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_03","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_04","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_05","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_06","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_07","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_08","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);
    Book ( TH1F("resMass_09","tautau-jet invariant mass (coll. nu);mass (#tau#tau,jet) [GeV]",30,0,3000), directory);


    // Book ( TH1F("HT","event HT;HT [GeV]",100,0,5000), directory);
    // Book ( TH1F("lheHT","lheHT;lheHT [GeV]",300,0,3000), directory);
    // Book ( TH1F("n_PV","number of primary vertices;n. of Primary Vertices",51,-.5,50.5), directory);

    // Book ( TH1F("jet_mass","jet softdrop mass (with JEC);jet softdrop mass [GeV]",50,0,250), directory);
...

{
    ...
    float iso_cut = -1.;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_n10", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = -.9;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_n09", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = -.8;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_n08", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = -.7;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_n07", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = -.6;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_n06", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = -.5;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_n05", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = -.4;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_n04", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = -.3;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_n03", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = -.2;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_n02", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = -.1;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_n01", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = .0;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_00", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = .9;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_09", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = .8;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_08", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = .7;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_07", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = .6;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_06", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = .5;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_05", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = .4;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_04", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = .3;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_03", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = .2;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_02", directory )->Fill(tauJetMass, b_weight );        

    iso_cut = .1;
    if ( (tau1_rawIso > iso_cut) && (tau2_rawIso > iso_cut) ) Hist( "resMass_01", directory )->Fill(tauJetMass, b_weight );        
    ...
    // Hist( "jet_mass", directory )->Fill( jetSoftdropMass , b_weight );
