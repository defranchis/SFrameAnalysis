...
void VHTausAnalysis::ExecuteEvent( const SInputData&, Double_t ) throw( SError ) 
{
    ...
    //std::cout<<m_eventInfo.averageIntPerXing->size()<<std::endl;
    //(*m_eventInfo.bunchCrossing)[v] == 0
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for ( int i = 0; i < (m_jetAK8.N); ++i ) 
    {
        ...
        if (passCuts && isJetID && massWindow && !isHbbtag)
...
void VHTausAnalysis::bookHistograms( const TString& directory ) {
    ...
    Book ( TH1F("PU_vertices","number of verices;n. of veritices",51,-.5,50.5), directory);
                                    ...
                                    TLorentzVector sumTau, TLorentzVector sumAll)
{
    ...
    //unsigned int nPuVtx = 0;

    // for (unsigned int k=0; k<m_eventInfo.averageIntPerXing->size(); ++k)
    //     if (m_eventInfo.bunchCrossing->at(k)==0) //std::cout<<m_eventInfo.averageIntPerXing->at(k)<<std::endl;
    //         nPuVtx = m_eventInfo.averageIntPerXing->at(k);

    //std::cout<<m_eventInfo.averageIntPerXing->size()<<std::endl;

    unsigned int nPuVtx = m_eventInfo.PV_N;
    Hist( "PU_vertices", directory )->Fill(nPuVtx,b_weight);
