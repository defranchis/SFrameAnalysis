...

{
    ...
    DeclareProperty( "jetMassMin",               m_jetMassMin         = 160.  );
...
void VHTausAnalysis::ExecuteEvent( const SInputData&, Double_t ) throw( SError ) 
{
    ...
    if (m_isSignal)
    {
        ...
        for (int iGenPart = 0; iGenPart <  m_genParticle.N; ++iGenPart)
        {
            ...
            int pdgId = myGenPart.pdgId();
            int n_Dau = myGenPart.nDau();
            int n_moth = myGenPart.nMoth();

            
            if ( abs(pdgId) == 15 && n_Dau > 1 && n_moth == 1)
            {

                int mother = abs(myGenPart.mother().at(0));

                if (! ( mother == 25 || mother == 15 ))
                    continue;

                bool skip = false;

                for (unsigned int iDau = 0; iDau < myGenPart.dau().size(); ++iDau)
                {
                    if (abs(myGenPart.dau().at(iDau)) == 15)
                    {
                        skip = true;
                        break;
                    }
                ...
                if (!skip){
                    ...
                    for (unsigned int iDau = 0; iDau < myGenPart.dau().size(); ++iDau)
                    {
                        int dau = abs(myGenPart.dau().at(iDau));
                        if (dau==11 || dau==13)
                        {
                            ++n_lepTaus;
                            break;
            ...
            } //first check on pdgId, daughters and mothers
    ...
    int n_CSV_tags = goodAK04_CSV_M.size();

    if ( !(( n_CSV_tags == 1 )||( n_CSV_tags == 2 )) )
        return;
...
/////////////////////////////////////////////////
    ...
    for ( int i = 0; i<(m_electron.N); ++i )
    {
        ...
        //bool isElectronID = myelectron.isLooseElectron();
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for ( int i = 0; i<(m_tau.N); ++i )
    {
        ...
        if (passCuts && isTauDM && isTauBoosted && isTauIso)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (m_isSignal && n_lepTaus >= 0){
        fillHistograms(m_catNames[n_lepTaus+1], selectedJet, selectedTau_1, selectedTau_2, goodMet, 
                       sumTauTLV_sel, sumAllTLV_sel, goodAK04_CSV_M);
    }
...
void VHTausAnalysis::bookHistograms( const TString& directory ) {
    ...
    Book ( TH1F("n_lep_taus","number leptonic taus; n. lep taus",3,-.5,2.5), directory);
                                     ...
                                     const std::vector<UZH::Jet> & goodAK04_CSV_M)
{
    ...
    Hist("n_lep_taus", directory )->Fill(n_lepTaus, b_weight);
