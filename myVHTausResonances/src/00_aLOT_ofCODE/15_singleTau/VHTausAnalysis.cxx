...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // now apply collinear neutrino approx
...
void VHTausAnalysis::bookHistograms( const TString& directory ) {
    ...
    Book ( TH1F("jet_mass","Hbb jet pruned mass (JEC);jetMass (GeV)",40,0,200), directory);
    Book ( TH1F("jet_pT","Hbb jet pT;pT (GeV)",30,0,3000), directory);
    Book ( TH1F("jet_eta","Hbb jet pseudorapidity ;#eta",20,-2.4,2.4), directory);
    Book ( TH1F("jet_phi","Hbb jet azimuthal angle;#phi",20,-TMath::Pi(),TMath::Pi()), directory);
    Book ( TH1F("jet_tau21","Hbb jet subjettiness ratio tau21; #tau21",20,0,1.1), directory);
    Book ( TH1F("jet_Hbbtag","Hbb jet pruned mass (JEC);jetMass (GeV)",20,-1,1), directory);

    Book ( TH1F("tau1_pT","leading tau pT;pT (GeV)",30,0,2000), directory);
    Book ( TH1F("tau1_eta","leading tau pseudorapidity ;#eta",20,-2.4,2.4), directory);
    Book ( TH1F("tau1_phi","leading tau azimuthal angle;#phi",20,-TMath::Pi(),TMath::Pi()), directory);

    // Book ( TH1F("tau2_pT","trailing tau pT;pT (GeV)",30,0,700), directory);
    // Book ( TH1F("tau2_eta","trailing tau pseudorapidity ;#eta",20,-2.4,2.4), directory);
    // Book ( TH1F("tau2_phi","trailing tau azimuthal angle;#phi",20,-TMath::Pi(),TMath::Pi()), directory);

    Book ( TH1F("MET_ET","missing energy ET;ET (GeV)",30,0,2000), directory);
    Book ( TH1F("MET_px","missing energy px;px (GeV)",20,0,1000), directory);
    Book ( TH1F("MET_py","missing energy py;py (GeV)",20,0,1000), directory);

    Book ( TH1F("jetmet_DP","jet-met angle;#Delta#phi(jet,MET)",20,0,TMath::Pi()), directory);
    Book ( TH1F("taumet_DP","tau-met angle;#Delta#phi(#tau,MET)",20,0,TMath::Pi()), directory);
    
    // Book ( TH1F("tautau_mass","tau pair invariant mass (coll. nu);m(#tau,#tau) (GeV)",30,0,250), directory);
    // Book ( TH1F("tautau_pT","tau pair pT (coll. nu);#tau pair pT (GeV)",30,0,3000), directory);
    // Book ( TH1F("tautau_eta","tau pair pseudorapidity (coll. nu);#eta",20,-2.4,2.4), directory);
    // Book ( TH1F("tautau_DR","tau pair distance;#DeltaR(#tau,#tau)",20,0,2), directory);
    Book ( TH1F("taujet_DR","tau-jet distance;#DeltaR(#tau,jet) ",30,0,6), directory);
                                    ...
                                    TLorentzVector sumTau, TLorentzVector sumAll)
{
    ...
    Hist( "jet_phi", directory )->Fill( Jet.phi() , b_weight);
    ...
    float tauJetDR = tau1.DeltaR(jet);
    ...
    TVector2 v2_tau (tau1.Px(),tau2.Py());
    TVector2 v2_jet (jet.Px(),jet.Py());
    
    Hist( "jetmet_DP", directory )->Fill(fabs(v2_met.DeltaPhi(v2_jet)),b_weight);
    Hist( "taumet_DP", directory )->Fill(fabs(v2_met.DeltaPhi(v2_tau)),b_weight);
