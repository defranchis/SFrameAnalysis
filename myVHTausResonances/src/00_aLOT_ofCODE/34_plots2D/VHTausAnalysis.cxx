...
// ////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for ( int i = 0; i < (m_jetAK8.N); ++i ) 
    {
        ...
        if (passCuts && isJetID && isHbbtag && massWindow)
...
void VHTausAnalysis::bookHistograms( const TString& directory ) {
    ...
    Book ( TH2F("mjet_vs_mtaus","tau mass vs jet mass (coll. nu);jet pruned mass (GeV);m(#tau,#tau) coll. nu (GeV) ",25,50,250,25,0,250), directory);
    Book ( TH1F("diff_sum_pt","(p_{T}^{(bb)}-p_{T}^{(#tau#tau)})/(p_{T}^{(bb)}+p_{T}^{(#tau#tau)});(p_{T}^{(bb)}-p_{T}^{(#tau#tau)})/(p_{T}^{(bb)}+p_{T}^{(#tau#tau)});",60,-1,2), directory);
    Book ( TH1F("ratio_pt","p_{T}^{(#tau#tau)}/p_{T}^{(bb)};p_{T}^{(#tau#tau)}/p_{T}^{(bb)};",40,0,3), directory);
                                    ...
                                    TLorentzVector sumTau, TLorentzVector sumAll)
{
    ...
    Hist( "diff_sum_pt", directory )->Fill((jet.Pt()-sumTau.Pt())/(jet.Pt()+sumTau.Pt()),b_weight);
    Hist( "ratio_pt", directory )->Fill( sumTau.Pt()/jet.Pt() , b_weight );

    Retrieve<TH2F>( "mjet_vs_mtaus", directory )->Fill( (Double_t)Jet.pruned_massCorr(), (Double_t)tautauMass, (Double_t)b_weight);
    //Hist( "mjet_vs_mtaus", directory )->Fill(Jet.pruned_massCorr(),tautauMass);
