...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for ( int i = 0; i < (m_jetAK8.N); ++i ) 
    {
        ...
        bool no_massWindow = (prunedMass < m_jetMassMin) && (prunedMass > 50.);
        bool isHbbtag = (myjet.Hbbtag() < m_jetHbbtagMin);
        //bool isMetAngle = (deltaPhi > m_jetMetDeltaPhiMin);

        if (passCuts && isJetID && no_massWindow && isHbbtag /*&& isMetAngle*/)
...
void VHTausAnalysis::bookHistograms( const TString& directory ) {
    ...
    Book ( TH1F("jet_mass","Hbb jet pruned mass (JEC);jetMass (GeV)",50,0,200), directory);
    Book ( TH1F("jet_pT","Hbb jet pT;pT (GeV)",50,0,3000), directory);
    Book ( TH1F("jet_eta","Hbb jet pseudorapidity ;#eta",50,-3,3), directory);
    Book ( TH1F("jet_phi","Hbb jet azimuthal angle;#phi",20,-4,4), directory);
    Book ( TH1F("jet_tau21","Hbb jet subjettiness ratio tau21; #tau21",50,0,1.5), directory);
    Book ( TH1F("jet_Hbbtag","Hbb jet pruned mass (JEC);jetMass (GeV)",60,-1.2,1.2), directory);
    ...
    Book ( TH1F("MET_px","missing energy px;px (GeV)",30,0,1500), directory);
    Book ( TH1F("MET_py","missing energy py;py (GeV)",30,0,1500), directory);

    Book ( TH1F("jetmet_DP","jet-met angle;#Delta#phi(jet,MET)",30,0,TMath::Pi()), directory);
    Book ( TH1F("tautaumet_DP","tautau-met angle (coll. nu);#Delta#phi(#tau#tau,MET)",30,0,TMath::Pi()), directory);
    
    Book ( TH1F("tautau_mass","tau pair invariant mass (coll. nu);m(#tau,#tau) (GeV)",40,0,200), directory);
    Book ( TH1F("tautau_pT","tau pair pT (coll. nu);#tau pair pT (GeV)",50,0,3000), directory);
    Book ( TH1F("tautau_eta","tau pair pseudorapidity (coll. nu);#eta",50,-3,3), directory);
    Book ( TH1F("tautau_DR","tau pair distance;#DeltaR(#tau,#tau)",30,0,2), directory);
    Book ( TH1F("taujet_DR","tautau-jet distance (coll. nu);#DeltaR(#tau#tau,jet) ",40,0,6), directory);
