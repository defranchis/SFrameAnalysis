...
void VHTausAnalysis::ExecuteEvent( const SInputData&, Double_t ) throw( SError ) 
{
   ...
   vector<TString> trigDenom;
   // trigDenom.push_back("PFHT400_v");
   trigDenom.push_back("IsoMu18_v");
   // trigDenom.push_back("HLT");
   // trigDenom.push_back("CaloJet500_NoJetID");


   for (std::map<std::string,bool>::iterator it = (m_eventInfo.trigDecision)->begin(); it != (m_eventInfo.trigDecision)->end(); ++it){
       for (unsigned int t = 0; t < trigDenom.size(); ++t ){
           if ((it->first).find(trigDenom.at(t)) != std::string::npos) {
               // cout<<it->first<<endl;
               if (it->second == true) passDenom = true; 
           }
       }
   }
   ...
   if (!passDenom) return;
    ...
    // if (m_isData){
    //     for (std::map<std::string,bool>::iterator it = (m_eventInfo.trigDecision)->begin(); it != (m_eventInfo.trigDecision)->end(); ++it){
    //         if ((it->first).find("PFHT400_v") != std::string::npos) 
...
void VHTausAnalysis::bookHistograms( const TString& directory ) {
    ...
    Book ( TEfficiency("eff_HT_verycoarse","trigger efficiency vs HT;event HT [GeV]",20,500,1500), directory);
    Book ( TEfficiency("eff_HT_coarse","trigger efficiency vs HT;event HT [GeV]",50,500,1500), directory);
    ...
    Book ( TEfficiency("eff_HT_insanely_coarse","trigger efficiency vs HT;event HT [GeV]",2,0,3000), directory);
...
{
    Retrieve<TEfficiency>("eff_HT_verycoarse", directory)->Fill(pass_trigger, event_HT);
    Retrieve<TEfficiency>("eff_HT_coarse", directory)->Fill(pass_trigger, event_HT);
    ...
    Retrieve<TEfficiency>("eff_HT_insanely_coarse", directory)->Fill(pass_trigger, event_HT);
