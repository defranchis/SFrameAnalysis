...
int main ( int argc, char * argv[] ){
    ...
    TString pathToFiles = "~/HH/8_X_Framework/Analysis/SFrameAnalysis/rootFiles/code_v7/";
    ...
    for (unsigned int iSample = 0; iSample < sampleList.size(); ++iSample)
    {
        ...
        // if (sampleName.Contains("ZZ")) xSection = 16.523 ;
            ...
            if (sampleName.Contains("Radion")) xSection = .1 ;
        ...
        for (unsigned int iHisto = 0; iHisto < listOfHists.size(); ++iHisto)
        {
            ...
            if (iHisto==0 && !sampleName.Contains("JetHT")) cout<<setw(40)<<sampleName<<"\t"<<setw(10)<<eventsInHisto<<"\t"<<
    ...
    leg->AddEntry(histos_Radion.at(0),"sig. M=2TeV #sigma=100fb","f");
    ...
    for (unsigned int iHisto = 0; iHisto < listOfHists.size(); ++iHisto)
    {
        ...
        if (histName.Contains("softdrop") || histName.Contains("pruned"))
        {
                    
            int minBin = histos_Data.at(iHisto)->FindBin(65);
        ...
        if ( histName=="HT" )
        {
            TFile * aOutFile = new TFile ("HT_correction.root", "RECREATE");
            h_ratio->SetName("HT_correction");
            h_ratio->Write();
            aOutFile->Close();
            delete aOutFile;
        }
        ...
        p_stack->Draw();
        if (!isBlindVariable) p_ratio->Draw();
        ...
        if ( histName.Contains("tautau_pT") ){
            xmin = 0.;
            xmax = 2000.;
        }

        if ( histName.Contains("tau_tau_DR") ){
            xmin = 0.;
            xmax = 6.;
        }

        if ( histName.Contains("original_tautau_jet_DR") ){
            xmin = 0.;
            xmax = 8.5;
        }

        if ( histName.Contains("corrected_tautau_jet_DR") ){
            xmin = 0.;
            xmax = 15.;
        }
        ...
        Stacks.at(iHisto)->SetMinimum(max/1E05);
        Stacks.at(iHisto)->SetMaximum(2.5*max);
        ...
        else if (histName=="jet_mass")
            Stacks.at(iHisto)->GetXaxis()->SetTitle("AK08 softdrop mass [GeV]");
