...
int main ( int argc, char * argv[] ){
    ...
    //protected scope
    {
        ...
        if (inFile->IsZombie()){
            cout<<endl<<"missing input file "<<sampleName<<endl<<endl;
            return 0;
        }
    ...
    for (unsigned int iDir = 0; iDir<listOfDirs.size(); ++iDir)
    {
        ...
        for (unsigned int iSample = 0; iSample < sampleList.size(); ++iSample)
        {
            ...
            double integratedLuminosity = 36.22 * 1E03;   // in pb^-1
            ...
            if ( ((name == "20_code_v18") || (name == "21_code_v19")) && ((!isData) && (!isSignal)) )
            {
                double MC_corr_fact = 1.;
                
                TFile * inData = new TFile (pathToFiles+"/VHTausAnalysis.JetHT.root") ;
                TH1F * data_preCut = (TH1F*) inData->GetObjectChecked("preHbbCut_check_noTauIso/jet_Hbbtag","TH1F");
                TH1F * data_postCut = (TH1F*) inData->GetObjectChecked("4_check_noTauIso/jet_Hbbtag","TH1F");

                double dataInt_preCut = data_preCut->Integral();
                double dataInt_postCut = data_postCut->Integral();
                
                TH1F * this_preCut = (TH1F*) inFile->GetObjectChecked("preHbbCut_check_noTauIso/jet_Hbbtag","TH1F");
                TH1F * this_postCut = (TH1F*) inFile->GetObjectChecked("4_check_noTauIso/jet_Hbbtag","TH1F");

                double thisInt_preCut = this_preCut->Integral();
                double thisInt_postCut = this_postCut->Integral();

                if( (dataInt_preCut > 0) && (thisInt_postCut > 0) ) 
                    MC_corr_fact = (dataInt_postCut/dataInt_preCut)*(thisInt_preCut/thisInt_postCut);

                // cout<<sampleName<<":\t corr. fact. = "<<MC_corr_fact<<endl;

                if (!dirName.Contains("preHbbCut")) scaleEventWeight *= MC_corr_fact ;
        ...
        for (unsigned int iHisto = 0; iHisto < listOfHists.size(); ++iHisto)
        {
            ...
            if (name=="17_code_v15_noHbbLowerCut" && dirName=="4_check_noTauIso" && histName=="jet_Hbbtag")
            {
                TFile * aOutFile = new TFile ("HbbTag_correction.root", "RECREATE");
                h_ratio->SetName("HbbTag_correction");
                h_ratio->Write();
                aOutFile->Close();
                delete aOutFile;
            }
            ...
            latexLabel2.SetNDC();
            latexLabel2.DrawLatex(0.72, 0.96, "36.2 fb^{-1} (13 TeV)");


            TPad * p_stack = new TPad ("","",0.0,0.2,1.0,.95);
            TPad * p_ratio = new TPad ("","",0.0,0.0,1.0,0.2);

            if (dirName.Contains("_SR") || dirName.Contains("_bveto")) p_stack = new TPad ("","",0.0,0.0,1.0,.95);



            p_stack->Draw();
            if (!dirName.Contains("_SR") && !dirName.Contains("_bveto")) {
                p_ratio->Draw();
                p_ratio->SetGrid();
            }
            p_stack->cd();
            ...
            if (!dirName.Contains("_SR") && !dirName.Contains("_bveto")) histos_Data.at(iHisto)->Draw("same");
            ...
            if (histName=="resonance_mass_bin")
            {
                ...
                vector<double> mass_point {0,1000,1200,1400,1800,2000,2500,3000,3500,4000,4500};
                
                if ( temp_dirName=="0_SR" || temp_dirName=="1_bveto" || temp_dirName=="2_tauveto" || temp_dirName=="3_allveto" )
                {

                    for (unsigned int iMassPoint = 0; iMassPoint < mass_point.size(); ++iMassPoint)
                    {

                        double massPoint = mass_point.at(iMassPoint);
                        TString s_massPoint; s_massPoint.Form("%i",(int)massPoint);

                        if (s_massPoint == "0") s_massPoint = "integral";

                        ofstream outDY ("txtFiles/"+name+"/DY_"+temp_dirName+"_"+s_massPoint+".txt", ios::out);
                        ofstream outWJ ("txtFiles/"+name+"/WJets_"+temp_dirName+"_"+s_massPoint+".txt", ios::out);
                        ofstream outTT ("txtFiles/"+name+"/TT_"+temp_dirName+"_"+s_massPoint+".txt", ios::out);

                        int iMin = -1;
                        int iMax = -1;

                        if ( massPoint == 0 ){
                            iMin = histos_Data.at(iHisto)->FindBin(800.);
                            iMax = histos_Data.at(iHisto)->GetNbinsX();
                        
                        }

                        else if (massPoint >= 3000){
                            iMin = histos_Data.at(iHisto)->FindBin(3000*.85);
                            iMax = histos_Data.at(iHisto)->FindBin(4500*1.15);
                        }

                        else {

                            iMin = histos_Data.at(iHisto)->FindBin( massPoint * .85 );
                            iMax = histos_Data.at(iHisto)->FindBin( massPoint * 1.15 );
                        }
                        ...
                        // outTxt<<"QCD\t"<<massMin<<"\t"<<massMax<<"\t"<<estimatedQCD<<"\t"<<err_estimatedQCD<<endl;
                        ...
                        outTT<<TTIntegral<<"\t"<<err_TTIntegral<<endl;
                        outWJ<<WJetsIntegral<<"\t"<<err_WJetsIntegral<<endl;
                        outDY<<DYIntegral<<"\t"<<err_DYIntegral<<endl;
                ...
                } // if region we are interested in
            }
        }            
