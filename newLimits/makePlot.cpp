...
int main( int argc, char * argv[] ){
    ...
    graph_1->SetFillColorAlpha(kGreen,.5);
    graph_2->SetFillColorAlpha(kYellow,.5);
    ...
    // latexLabel3.DrawLatex(0.17, 0.96, "Preliminary");
    ...
    latexLabel2.DrawLatex(0.68, 0.96, "36.2 fb^{-1}   #sqrt{s} = 13 TeV");
    // latexLabel2.DrawLatex(0.62, 0.96, "30 fb^{-1} extrapolation (13 TeV)");
    ...
    multi->Draw("a3");
    // multi->Draw("a4");
    multi->GetXaxis()->SetRangeUser(min,3000);
    multi->GetYaxis()->SetRangeUser(2.E-3,2);
    // multi->GetYaxis()->SetRangeUser(7E-3,2);
    c->Modified(); c->Update();
    graph->Draw("PLsame");
    ...
    TFile * outFile = new TFile ("plot/"+name+"/exclusionPlot.root","RECREATE");
    c->Write();
    graph->Write();
    outFile->Close();
    c->Print("plot/"+name+"/plot.pdf","pdf");
    c->Print("plot/"+name+"/plot.png","png");
