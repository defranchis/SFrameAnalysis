...
int main ( int argc, char * argv[] ){
    ...
    for (unsigned int iDir = 0; iDir<listOfDirs.size(); ++iDir)
    {
        ...
        // vector<vector<double>> error_MC_xS ;
        // vector<vector<double>> error_TT_xS ;
        // vector<vector<double>> error_WJets_xS ;
        // vector<vector<double>> error_DY_xS ;
        // vector<vector<double>> error_ST_xS ;
        // vector<vector<double>> error_Diboson_xS ;
        ...
        for (unsigned int iHisto = 0; iHisto < listOfHists.size(); ++iHisto)
        {
            ...
            if (histName=="resonance_mass_bin")
            {
                ...
                ofstream outTxt ("txtFiles/"+name+"/"+dirName+"_binned.txt", ios::out);
                ...
                for (unsigned int iMassBin = 0; iMassBin < mass_range.size()+1; ++iMassBin)
                {
                    ...
                    // double subleadingBkgIntegral = 
                    //     histos_DY.at(iHisto)->Integral(iMin,iMax-1) +
                    //     histos_ST.at(iHisto)->Integral(iMin,iMax-1) +
                    //     histos_Diboson.at(iHisto)->Integral(iMin,iMax-1) ;
                    ...
                    // double err_TTIntegral_xS = 0;
            
                    for (int iBin = iMin-1; iBin<iMax-1; ++iBin){
                        err_TTIntegral_2 += error_TT.at(iHisto).at(iBin);
                        // err_TTIntegral_xS += error_TT_xS.at(iHisto).at(iBin);
                    }

                    double err_WJetsIntegral_2 = 0;
                    // double err_WJetsIntegral_xS = 0;
            
                    for (int iBin = iMin-1; iBin<iMax-1; ++iBin){
                        err_WJetsIntegral_2 += error_WJets.at(iHisto).at(iBin);
                        // err_WJetsIntegral_xS += error_WJets_xS.at(iHisto).at(iBin);
                    }

                    double err_STIntegral_2 = 0;
                    // double err_STIntegral_xS = 0;
            
                    for (int iBin = iMin-1; iBin<iMax-1; ++iBin){
                        err_STIntegral_2 += error_ST.at(iHisto).at(iBin);
                        // err_STIntegral_xS += error_ST_xS.at(iHisto).at(iBin);
                    }

                    double err_DYIntegral_2 = 0;
                    // double err_DYIntegral_xS = 0;
            
                    for (int iBin = iMin-1; iBin<iMax-1; ++iBin){
                        err_DYIntegral_2 += error_DY.at(iHisto).at(iBin);
                        // err_DYIntegral_xS += error_DY_xS.at(iHisto).at(iBin);
                    }
                    ...
                    for (int iBin = iMin-1; iBin<iMax-1; ++iBin){
                        ...
                        // err_DibosonIntegral_xS += error_Diboson_xS.at(iHisto).at(iBin);
                    }
                    ...
                    // double err_TTIntegral_xS = pow(err_TTIntegral_2_xS , .5);
                    // double err_WJetsIntegral_xS = pow(err_WJetsIntegral_2_xS , .5);
                    // double err_STIntegral_xS = pow(err_STIntegral_2_xS , .5);
                    // double err_DYIntegral_xS = pow(err_DYIntegral_2_xS , .5);
                    // double err_DibosonIntegral_xS = pow(err_DibosonIntegral_2_xS , .5);
                    ...
                    if (!dirName.Contains("_SR")){
                        ...
                        // outTxt<<"sub\t"<<massMin<<"\t"<<massMax<<"\t"<<subleadingBkgIntegral<<"\t"<<err_subleadingBkgIntegral<<endl;
